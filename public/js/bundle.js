...
var displayMap = exports.displayMap = function displayMap(locations) {
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if (mapElement) {
      var _locations = [];
      try {
        _locations = JSON.parse(mapElement.dataset.locations);
        console.log('Parsed locations:', _locations);
      } catch (error) {
        ...
        _locations.forEach(function (loc) {
          ...
          if (loc.coordinates && Array.isArray(loc.coordinates) && loc.coordinates.length === 2) {
            ...
            if (!isNaN(lng) && !isNaN(lat)) {
              ...
              console.log("Added marker at [".concat(lng, ", ").concat(lat, "]"));
              new mapboxgl.Popup().setLngLat(loc.coordinates).setHTML("<p> ".concat(loc.day, ": ").concat(loc.description, "<o>"));
